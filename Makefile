...
# External build configuration
 ...
 # External libc interface
  # <The value is used for testing Nuttx build.
  #  Variable could be separated into several variables with values
  #  for testing targets for different operating systems.>
...
export TARGET_<OS-NAME>_SYSTEMS_MODS = $(TARGET_<OS-NAME>_SYSTEMS) \
                                       $(foreach __MOD,$(TARGET_<OS-NAME>_MODS),$(foreach __SYSTEM,$(TARGET_<OS-NAME>_SYSTEMS),$(__SYSTEM)-$(__MOD)))
...
export JERRY_<OS-NAME>_TARGETS = $(foreach __MODE,$(TARGET_DEBUG_MODES),$(foreach __SYSTEM,$(TARGET_<OS-NAME>_SYSTEMS_MODS),$(__MODE).$(__SYSTEM))) \
                                 $(foreach __MODE,$(TARGET_RELEASE_MODES),$(foreach __SYSTEM,$(TARGET_<OS-NAME>_SYSTEMS_MODS),$(__MODE).$(__SYSTEM)))
...
export JERRY_TARGETS = $(JERRY_LINUX_TARGETS) $(JERRY_NUTTX_TARGETS) $(JERRY_<OS-NAME>_TARGETS) $(JERRY_STM32F3_TARGETS) $(JERRY_STM32F4_TARGETS) unittests
...
# Build directories
 ...
 # <os-name>
  BUILD_DIRS_<OS-NAME> := $(foreach _OPTIONS_COMBINATION,$(OPTIONS_COMBINATIONS),$(BUILD_DIR_PREFIX)$(_OPTIONS_COMBINATION)/<os-name>)
 # stm32f3
 ...
 # All together
  BUILD_DIRS_ALL := $(BUILD_DIRS_NATIVE) $(BUILD_DIRS_NUTTX) $(BUILD_DIRS_<OS-NAME>) $(BUILD_DIRS_STM32F3) $(BUILD_DIRS_STM32F4)
...
$(BUILD_DIRS_<OS-NAME>): prerequisites
	@ [ "$(EXTERNAL_LIBC_INTERFACE)" != "" ] && [ -x `which $(EXTERNAL_C_COMPILER)` ] && [ -x `which $(EXTERNAL_CXX_COMPILER)` ] || \
          (echo "Wrong external arguments."; \
           echo "EXTERNAL_LIBS_INTERFACE='$(EXTERNAL_LIBS_INTERFACE)'"; \
           echo "EXTERNAL_C_COMPILER='$(EXTERNAL_C_COMPILER)'"; \
...
$(JERRY_<OS-NAME>_TARGETS): $(BUILD_DIR)/<os-name>
	@ mkdir -p $(OUT_DIR)/$@
	@ [ "$(STATIC_CHECK)" = "OFF" ] || $(MAKE) -C $(BUILD_DIR)/<os-name> VERBOSE=1 cppcheck.$@ &>$(OUT_DIR)/$@/cppcheck.log || \
          (echo "cppcheck run failed. See $(OUT_DIR)/$@/cppcheck.log for details."; exit 1;)
	@ $(MAKE) -C $(BUILD_DIR)/<os-name> VERBOSE=1 $@ &>$(OUT_DIR)/$@/make.log || \
          (echo "Build failed. See $(OUT_DIR)/$@/make.log for details."; exit 1;)
	@ cp `cat $(BUILD_DIR)/<os-name>/$@/list` $(OUT_DIR)/$@/
...
$(BUILD_ALL)_<os-name>: $(BUILD_DIRS_<OS-NAME>)
	@ mkdir -p $(OUT_DIR)/$@
	@ $(MAKE) -C $(BUILD_DIR)/<os-name> plugins-all VERBOSE=1 &>$(OUT_DIR)/$@/make.log || \
          (echo "Build failed. See $(OUT_DIR)/$@/make.log for details."; exit 1;)
	@ $(MAKE) -C $(BUILD_DIR)/<os-name> $(JERRY_<OS-NAME>_TARGETS) VERBOSE=1 &>$(OUT_DIR)/$@/make.log || \
          (echo "Build failed. See $(OUT_DIR)/$@/make.log for details."; exit 1;)
...
build_all: $(BUILD_ALL)_native $(BUILD_ALL)_nuttx $(BUILD_ALL)_<os-name> $(BUILD_ALL)_stm32f3 $(BUILD_ALL)_stm32f4
