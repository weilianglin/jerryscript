 ...
 */
int
lit_charset_record_t::compare_zt (const ecma_char_t *str2, /**< buffer with string to compare */
                                  size_t n) /**< length of the string in buffer str2 */
{
  size_t i;
  if (get_length () == 0)
  {
  ...
  {
    ecma_char_t chr = it.read<ecma_char_t> ();
    if (chr > str2[i])
    {
      return 1;
    }
 ...
 */
bool
lit_charset_record_t::equal_zt (const ecma_char_t *s) /**< zero-terminated string */
{
 ...
 */
bool
lit_charset_record_t::equal_non_zt (const ecma_char_t *s, /**< string to compare with */
...
lit_literal_storage_t::create_charset_record (const ecma_char_t *s, /**< string to be placed in the record */
                                                                ...
                                                                * string */
{
  ...
  ret->set_alignment_bytes_count (lit_charset_record_t::size (buf_size) -
                                  (lit_charset_record_t::header_size () + buf_size));
  ...
  for (rcs_record_t *rec_p = lit_storage.get_first (); rec_p != NULL; rec_p = lit_storage.get_next (rec_p))
  {
    ...
    switch (rec_p->get_type ())
    {
      case LIT_STR:
      {
        ...
        it.skip (lit_charset_record_t::header_size ());
