...
lit_create_literal_from_charset (
  const ecma_char_t *s, /**< string to initialize the record, could be non-zero-terminated */
 ...
 */
literal_t
lit_find_literal_by_charset (const ecma_char_t *s, /**< a string to search for */
                             ecma_length_t len) /**< length of the string */
{
  for (literal_t lit = lit_storage.get_first (); lit != NULL; lit = lit_storage.get_next (lit))
  {
    ...
    if (type == LIT_STR_T)
    {
      ...
      if (!static_cast<lit_charset_record_t *>(lit)->compare_zt (s, len))
      {
        return lit;
      }
    }
    else if (type == LIT_MAGIC_STR_T)
    {
      const char *str = (const char *) ecma_get_magic_string_zt (
        static_cast<lit_magic_record_t *>(lit)->get_magic_str_id ());
      if (strlen (str) != len)
      {
        continue;
      }
 ...
 */
literal_t
lit_find_or_create_literal_from_charset (const ecma_char_t *s, /**< string, could be non-zero-terminated */
                                         ecma_length_t len) /**< length of the string */
{
  literal_t lit;
  lit = lit_find_literal_by_charset (s, len);
  if (lit == NULL)
  {
    lit = lit_create_literal_from_charset (s, len);
  }
  return lit;
 ...
 */
literal_t
lit_find_literal_by_num (ecma_number_t num) /**< number */
{
  for (literal_t lit = lit_storage.get_first (); lit != NULL; lit = lit_storage.get_next (lit))
  {
    rcs_record_t::type_t type = lit->get_type ();
...
lit_literal_equal_charset_rec (literal_t lit, /**< literal to compare */
                               lit_charset_record_t *rec) /**< charset record to compare */
{
  switch (lit->get_type ())
  {
    case LIT_STR_T:
    {
      return static_cast<lit_charset_record_t *>(lit)->equal (rec);
    }
    case LIT_MAGIC_STR_T:
    {
      return rec->equal_zt (ecma_get_magic_string_zt (static_cast<lit_magic_record_t *>(lit)->get_magic_str_id ()));
    }
    case LIT_NUMBER_T:
    {
      ecma_char_t buff[ECMA_MAX_CHARS_IN_STRINGIFIED_NUMBER];
      ecma_number_to_zt_string (static_cast<lit_number_record_t *>(lit)->get_number (), buff,
...
lit_literal_equal_zt (literal_t lit, /**< literal to compare */
                      const ecma_char_t *s) /**< zero-terminated string to compare */
{
  switch (lit->get_type ())
  {
    case LIT_STR_T:
    {
      return static_cast<lit_charset_record_t *>(lit)->equal_zt (s);
    }
    case LIT_MAGIC_STR_T:
    {
      return ecma_compare_zt_strings (s,
               ecma_get_magic_string_zt (static_cast<lit_magic_record_t *>(lit)->get_magic_str_id ()));
    }
    case LIT_NUMBER_T:
    {
      ecma_char_t buff[ECMA_MAX_CHARS_IN_STRINGIFIED_NUMBER];
  ...
  switch (lit2->get_type ())
  {
    ...
    case lit_literal_storage_t::LIT_MAGIC_STR:
    {
      return lit_literal_equal_zt (lit1, ecma_get_magic_string_zt (
                                           static_cast<lit_magic_record_t *>(lit2)->get_magic_str_id ()));
    }
...
lit_literal_equal_type_zt (literal_t lit, /**< literal to compare */
                           const ecma_char_t *s) /**< zero-terminated string */
{
 ...
 */
const ecma_char_t *
lit_literal_to_charset (literal_t lit, /**< literal */
