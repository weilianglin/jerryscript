                             ...
                             bool copy_bytecode) /**< byte code should be copied to memory */
{
  ...
  {
    if (is_trace_snapshot) printf("bytecode is copied to RAM!\n");
                                                   ...
                                                   * code is executed as global scope code) */
{
  ...
  if (is_trace_snapshot) printf("lit table size: %d\n", header_p->lit_table_size);
  ...
  if (is_trace_snapshot || is_trace_string) return JERRY_COMPLETION_CODE_OK;
